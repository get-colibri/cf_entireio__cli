...
func runMigrateCheckpointsV2(ctx context.Context, cmd *cobra.Command) error {
	...
	result, err := migrateCheckpointsV2(ctx, repo, v1Store, v2Store, gmetaStore, out)
	if err != nil {
		return err
	}
...
func migrateCheckpointsV2(ctx context.Context, repo *git.Repository, v1Store *checkpoint.GitStore, v2Store *checkpoint.V2GitStore, gmetaStore *checkpoint.GmetaStore, out io.Writer) (*migrateResult, error) {
	...
	for i, info := range v1List {
		...
		if migrateErr := migrateOneCheckpoint(ctx, repo, v1Store, v2Store, gmetaStore, info, out, prefix); migrateErr != nil {
			switch {
...
func migrateOneCheckpoint(ctx context.Context, repo *git.Repository, v1Store *checkpoint.GitStore, v2Store *checkpoint.V2GitStore, gmetaStore *checkpoint.GmetaStore, info checkpoint.CommittedInfo, out io.Writer, prefix string) error {
	...
	for sessionIdx := range len(summary.Sessions) {
		...
		// Gmeta dual-write (session data only, no task metadata — see plan)
